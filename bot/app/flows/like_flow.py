    ...
    async def next_like_profile(self, message: Message, state: FSMContext) -> None:
        if message.from_user:
            ...
            if inbox_data:
                ...
                data = await self.user_service.get_user_profile_view(
                ...
                if data:
                    ...
                    if not photos:
                        await self.presenter.send_profile_without_photos(
                            message, data, more
                        )
                    else:
                    ...
                    if type_of_inbox == "MATCH":
                        await self.presenter.send_match(
                            message, candidate_id, data["name"]
                        )
                        await self.inbox_service.ack_inbox_item(
                            message.from_user.id, candidate_id
                        )
    ...
    async def swipe(self, message: Message, state: FSMContext, bot: Bot) -> None:
        ...
        if message.from_user:
            if message.text == "❤️" and candidate_id:
                ...
                await self.swipe_service.create_swipe(
                    message.from_user.id, candidate_id, True
                )
                ...
                await state.clear()
                await self.swipe_service.create_swipe(
                    message.from_user.id, candidate_id, False
                )
                await self.inbox_service.ack_inbox_item(
                    message.from_user.id, candidate_id
                )
