...
async def next_like_profile(message: Message, state: FSMContext) -> None:
    if message.from_user:
        if message.text == "🔥":
            await message.answer("Окей, давай посмотрим! 🚀", reply_markup=swipe_kb)
        ...
        if liker_id:
            ...
            if data:
                ...
                # --- 3. Save current profile id and name ---
            ...
            if message.text == "🔥":
                ...
                await message.answer("На сегодня все лайки закончились 🙃", reply_markup=main_kb)
...
async def swipe(message: Message, state: FSMContext, bot: Bot) -> None:
    ...
    if message.from_user:
        if message.text == "❤️" and liked_id:
            ...
            if count:
                await bot.send_message(liked_id, f"Эййй, у тебя взаимность ❤️))")
