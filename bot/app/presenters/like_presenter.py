...
class LikePresenter:
    ...
    async def send_profile_without_photos(self, message: Message, user: dict, more: int | None) -> None:
        caption = await self._create_profile_caption(user, more)
        await message.answer(caption, reply_markup=swipe_kb)
    
    async def send_profile(self, message: Message, user: dict, photos, more: int | None) -> None:
        caption = await self._create_profile_caption(user, more)
