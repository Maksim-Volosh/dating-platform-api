...
class DeckService:
    ...
    async def build_deck(self, telegram_id: int) -> None:
        ...
        users_pydantic = [UserCreate.model_validate(user, from_attributes=True) for user in users_models]
    ...
    async def get_next_user(self, telegram_id: int) -> UserCreate | None:
        ...
        if not user_bytes or not type(user_bytes) == bytes:
            ...
            if not user_bytes or not type(user_bytes) == bytes:   
                return None
        user_pydantic = UserCreate(**json.loads(user_bytes))
...
async def get_next_user_from_deck(
    ...
    user: UserCreate | None = await deck_service.get_next_user(telegram_id)
    if not user:
