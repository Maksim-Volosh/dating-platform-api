...
class RetrieveUserPhotosUseCase:
    ...
    async def execute(self, user: UserEntity) -> list[PhotoEntity]:
        photos: List[PhotoEntity] | None = await self.photo_repo.get_by_user_id(
            user.telegram_id
        )
        if photos is None:
...
class UpdateUserPhotosUseCase:
    def __init__(self, photo_repo: IPhotoRepository) -> None:
        self.photo_repo = photo_repo
    ...
    async def execute(
        ...
        file_ids: List[PhotoEntity] = await self.photo_repo.create(
            user.telegram_id, photos
        )
        return file_ids
...
class DeleteUserPhotosUseCase:
    ...
    async def execute(self, user: UserEntity) -> None:
        deleted_photos: List[PhotoEntity] | None = await self.photo_repo.delete(
            user.telegram_id
        )
