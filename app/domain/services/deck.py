...
class DeckBuilderService:
    ...
    async def build(self, user: UserEntity) -> None | List[UserEntity]:
        ...
        not_swiped_candidates = [
            candidate for candidate in candidates[:settings.deck.max_size] if await self.swipe_repo.was_swiped(user.telegram_id, candidate.telegram_id) is False
        ]
    ...
    async def _delete_user_from_others_decks(self, telegram_id: int, suspicious_users: List[UserEntity]) -> None:
            ...
            if user_deck is not None:
                for i, item in enumerate(user_deck):
                    if item.telegram_id == telegram_id:
                        ...
                        await self.cache.rpush(f"deck:{user.telegram_id}", user_deck, timeout=settings.deck.timeout)
    ...
    async def build_and_clean_others(self, user: UserEntity) -> None:
        ...
        if suspicious_users is not None:
            await self._delete_user_from_others_decks(user.telegram_id, suspicious_users)
